...
function generate_investment(strings, values) {
  ...
  for (var i = 0; i < strings.length; i++) {
    if (values[i] >= 0)
      colors +=
        "<div class='col s12 m2'><div class='card'><div class='card-content'><a class='btn-floating waves-effect waves-light green' style='width:100px;height:100px;margin-bottom:20px'><i class='material-icons' style='font-size:3rem; line-height:95px'>arrow_upward</i></a><p><h6>" +
        strings[i] +
        values[i] +
        "</h6></p></div></div></div>";
    else
      colors +=
        "<div class='col s12 m2'><div class='card'><div class='card-content'><a class='btn-floating waves-effect waves-light red' style='width:100px;height:100px;margin-bottom:20px'><i class='material-icons' style='font-size:3rem; line-height:95px'>arrow_downward</i></a><p><h6>" +
        strings[i] +
        values[i] +
        "</h6></p></div></div></div>";
  }
...
function completeFn(results) {
  ...
  for (var i = 1; i < csv.length; i++) {
    if (
      !isNaN(csv[i][indeces["open"]]) &&
      !isNaN(csv[i][indeces["close"]]) &&
      !isNaN(csv[i][indeces["low"]]) &&
      !isNaN(csv[i][indeces["high"]]) &&
      !isNaN(csv[i][indeces["volume"]])
    ) {
      stocks.push([
        parseFloat(csv[i][indeces["open"]]),
...
$("#uploadcsv").change(function () {
  ...
  if ($(this).val().search(".csv") <= 0) {
    $(this).val("");
    Materialize.toast("Only supports CSV", 4000);
    return;
  }
  $(this).parse({
    config: config,
  });
  Materialize.toast("File Upload complete!", 5000);
});
...
$("#trainbutton").click(function () {
  ...
  if (
    parseFloat($("#inputdropoutrate").val()) < 0 ||
    parseFloat($("#inputdropoutrate").val()) > 1
  ) {
    Materialize.toast("input dropout must bigger than 0 and less than 1", 4000);
    return;
  }
  if (
    parseFloat($("#smooth").val()) < 0 ||
    parseFloat($("#smooth").val()) > 1
  ) {
    Materialize.toast(
      "smoothing weights must bigger than 0 and less than 1",
      4000
    );
    return;
  }
  if (
    parseFloat($("#outputdropoutrate").val()) < 0 ||
    parseFloat($("#outputdropoutrate").val()) > 1
  ) {
    Materialize.toast(
      "output dropout must bigger than 0 and less than 1",
      4000
    );
    return;
  }
  setTimeout(function () {
    ...
    X_scaled = minmax_scaled.scaled.slice(
      [0],
      [Math.floor(minmax_scaled.scaled.shape[0] / timestamp) * timestamp + 1]
    );
    ...
    function f(x, states) {
      ...
      last_sequences = dropout_nn(
        forward[0].reshape([x.shape[1], parseInt($("#sizelayer").val())]),
        parseFloat($("#outputdropoutrate").val())
      );
      return {
    ...
    batch_states = [
      tf.zeros([1, parseInt($("#sizelayer").val())]),
      tf.zeros([1, parseInt($("#sizelayer").val())]),
    ];
    (arr_loss = []), (arr_layer = []);
    function async_training_loop(callback) {
      (function loop(i) {
        ...
        for (
          var k = 0;
          k < Math.floor(X_scaled.shape[0] / timestamp) * timestamp;
          k += timestamp
        ) {
          ...
          optimizer.minimize(() =>
            cost(batch_y, f(batch_x, batch_states)["forward"])
          );
          total_loss += parseFloat(
            cost(batch_y, f(batch_x, batch_states)["forward"])
              .toString()
              .slice(7)
          );
        ...
        output_predict
          .slice([0, 1], null)
          .assign(tf_str_tolist(X_scaled.slice(0, 1))[0], false);
        ...
        for (
          ...
          output_predict
            .slice([k + 1, k + 1 + timestamp], null)
            .assign(state_forward, false);
          batch_states = [feed.states_1, feed.states_2];
        }
        batch_x = X_scaled.slice([upper_b], [distance_upper_b]).reshape([
          1, -1, 1,
        ]);
        feed = f(batch_x, batch_states);
        state_forward = tf_nj_list(feed.forward);
        output_predict
          .slice([upper_b + 1, X_scaled.shape[0] + 1], null)
          .assign(state_forward, false);
        ...
        for (var k = 0; k < future - 1; k += 1) {
          batch_x = tf
            .tensor(tensor_output_predict.slice(pointer - timestamp, pointer))
            .reshape([1, -1, 1]);
          feed = f(batch_x, batch_states);
        ...
        $("#log").append(
          "Epoch: " + (i + 1) + ", avg loss: " + total_loss + "<br>"
        );
        predicted_val = tf_nj_list_flatten(
          reverse_minmax_1d(
            tf.tensor(tensor_output_predict),
            minmax_scaled["min"],
            minmax_scaled["max"]
          )
        );
        predicted_val = smoothing_line(
          predicted_val,
          parseFloat($("#smooth").val())
        );
    ...
    async_training_loop(function () {
      ...
      my_investment = simple_investor(
        close,
        predicted_val,
        parseInt($("#history").val()),
        parseFloat($("#initialmoney").val()),
        parseInt($("#maxbuy").val()),
        parseInt($("#maxsell").val()),
        new_date
      );
      $("#table-body").html("");
...
function plot_pairplot(val) {
  ...
  for (var i = 0; i < val["movement_changes"].length; i++) {
    ...
    for (var k = 0; k < val["movement_changes"][i]["movement"].length; k++) {
      data.push([
        val["movement_changes"][i]["date"][k],
        val["movement_changes"][i]["movement"][k],
      ]);
    }
    grids.push({
      show: true,
    ...
    titles.push({
      textAlign: "center",
      text:
        val["movement_changes"][i]["title"] +
        " " +
        val["pct_changes"][i].toFixed(2) +
        "%",
      textStyle: {
        fontSize: 12,
        fontWeight: "normal",
  ...
  generate_investment(
    ...
    [
      ...
      stock_changes.toFixed(2),
      stock_changes_percent.toFixed(2),
      (val["gain_crude_oil"] * 100).toFixed(2),
      (val["gain_gold"] * 100).toFixed(2),
    ]
  );
}
